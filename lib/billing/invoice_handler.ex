defmodule Billing.InvoiceHandler do
  ...
  def handle_invoice(invoice_id) do
    ...
    with {:ok, invoice_params} <- Invoicing.build_request_params(invoice),
         ...
         {:ok, electronic_invoice} <- ElectronicInvoices.create_electronic_invoice(invoice.id, access_key),
         ...
         {:ok, emission_profile} <- increment_emission_profile_sequence(invoice.emission_profile_id) do
      {:ok, emission_profile}
    else
      {:error, error} ->
        {:error, error}
